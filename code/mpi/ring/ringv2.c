...
static void ring_reduce(const int *sendbuf, int *recvbuf, MPI_Comm comm)
{
  ...
  for (int i=0; i<size-1; i++){
       int rank_send = (rank+i+1) % size;
       int rank_recv = (rank-i-1+size) % size;

       // printf("Process %d attempting to send to process %d and receive from process %d\n", rank, rank_send, rank_recv);
